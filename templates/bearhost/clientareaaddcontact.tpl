...
<div class="">
<p>{$LANG.clientareachoosecontact}: <select name="contactid" onchange="submit()" class="selectbar">
    {foreach item=contact from=$contacts}
        <option value="{$contact.id}">{$contact.name} - {$contact.email}</option>
    {/foreach}
    <option value="new" selected="selected">{$LANG.clientareanavaddcontact}</option>
    </select><input class="readmore_but5 nofloat" style="padding:14px 24px;" type="submit" value="{$LANG.go}" /></p>
</div>
<div class="clear"></div>
</form>
